...
(function (jQuery, window, document, undfined) {
  ...
  DragZoom.prototype = {
    ...
    wheel: function (ev, self) {
      if (
        self.options.zoom >= self.options.minzoom &&
        self.options.zoom <= self.options.maxzoom
      ) {
        //缩放开始回调
        self.options.onWheelStart &&
        typeof self.options.onWheelStart == "function"
          ? self.options.onWheelStart()
          : null;
        ...
        self.x =
          self.x -
          ((cursor_x - self.x) * (self.options.zoom - self.scale)) / self.scale;
        self.y =
          self.y -
          ((cursor_y - self.y) * (self.options.zoom - self.scale)) / self.scale;
        ...
        //缩放结束回调
        self.options.onWheelEnd && typeof self.options.onWheelEnd == "function"
          ? self.options.onWheelEnd()
          : null;
      }
      self.options.zoom =
        self.options.zoom < self.options.minzoom
    ...
    move: function (ev, self) {
      if (self.isMoved) {
        ...
        //拖拽移动回调
        self.options.onDragMove && typeof self.options.onDragMove == "function"
          ? self.options.onDragMove()
          : null;
      }
    },
      ...
      //拖拽结束回调
      self.options.onDragEnd && typeof self.options.onDragEnd == "function"
        ? self.options.onDragEnd()
        : null;
    },
  };
